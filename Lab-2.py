...
class FunctionCalculator(QWidget):
    ...
    def initUI(self):
        ...
        self.sin_cos_radio = QRadioButton('y = tg(3x) - tg(x) - 4sin(x)')
        self.exp_radio = QRadioButton('y = 23^(1+x^2) - 10^(1-x^3)')
    ...
    def calculate(self):
        if self.sin_cos_radio.isChecked():
            func = lambda x: np.tan(3*x) - np.tan(x) - 4*np.sin(x)
        elif self.exp_radio.isChecked():
            func = lambda x: 23**(1 + x**2) - 10**(1 - x**3)
        else:
            return
