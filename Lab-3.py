...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.function_combo.addItems(["y = tg(3x) - tg(x) - 4*sin(x)", "y = 23^(1+x^2) - 10^(1-x^3)"])
    ...
    def plot_graph(self):
        try:
            ...
            if selected_function == "y = tg(3x) - tg(x) - 4*sin(x)":
                y = np.tan(3*x) - np.tan(x) - 4*np.sin(x)
            elif selected_function == "y = 23^(1+x^2) - 10^(1-x^3)":
                y = 23**(1+x**2) - 10**(1-x**3)
